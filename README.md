...
初回起動時にキーボードレイアウトを選択することは可能ですが、入力イベントを処理する内部コードがそのレイアウト情報を参照していないためです。完全な多言語対応には大幅な改修が必要となるため、本リポジトリでは **「英語キーボードのマッピングをJIS用に修正、追加」** することで簡易対応を行いました。

## 主な変更点
- 英語キーボードと競合する部分の修正
- 日本語キーボード独自に関するキー部分の修正
- 「半角/全角」、「カタカナ/ひらがな」キーは Flags では正しくハンドリングできなかったため、WindowsMessage に変更

## 仕様・制限事項
**「カタカナ/ひらがな」キーはIMEがONの時は反応しません**。IMEがOFFの時は「カタカナ/ひらがな」キーを押すと反応しますが、IMEがONの時は反応しません。これはIME側でキー入力のイベントをアプリ側に渡さないように制御しているためです。

**Windows 11環境のみの対応**となります。Linux等の他OSにおける動作は未検証です。
...
本リポジトリをビルドするには **.NET 10 SDK** が必要です。ビルド方法は下記の「Want to build? Follow these instructions」に記載されています。

なお、以下のコマンドでリリースビルドを行うことで、実行環境に .NET 10 ランタイムがインストールされていないPCでも、単体で動作（Self-Contained）させることができます。
