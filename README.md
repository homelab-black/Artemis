...
## 仕様・制限事項
- 「カタカナ/ひらがな」キーはIMEがONの時は反応しません

IMEがOFFの時は「カタカナ/ひらがな」キーを押すと反応しますが、IMEがONの時は反応しません。これはIME側でキー入力のイベントをアプリ側に渡さないように制御しているためです。

- Windows 11 のみの対応：Windows 11環境でのみ動作確認を行っています

Linux等の他OSにおける動作は未検証です。
